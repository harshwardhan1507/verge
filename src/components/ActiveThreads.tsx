...
export function ActiveThreads({ className }: ActiveThreadsProps) {
  const unresolvedCount = useMemoryStore((state) => state.getUnresolvedCount());
  const commitmentCount = useMemoryStore((state) => 
    state.memories.filter(m => m.type === 'COMMITMENT' && !m.resolved).length
  );
